...
export const createGitHubSlice: StateCreator<AppState, [], [], GitHubSlice> = (set, get) => ({
  ...
  patchProjectIssueOrPr: async (cacheKey, rowId, updates) => {
    ...
    // Why: slug-only Project rows have no registered TeamRun repo, so fall back to the view source in the cache key, not the focused host.
  ...
  fetchWorkItems: async (
    ...
    const request = (async () => {
      ...
      try {
        ...
        // Why: stamp repoId at the fetch boundary so downstream consumers can rely on it — main doesn't know TeamRun's Repo.id.
