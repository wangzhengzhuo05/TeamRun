...
export type AgentStatusSlice = {
  ...
  /** Ephemeral launch snapshots keyed by pane; hook payloads lack TeamRun launch settings, so the renderer supplies them from startup. */
  ...
  return {
    ...
    setAgentStatus: (paneKey, payload, terminalTitle, timing, routing, metadata) => {
      ...
      set((s) => {
        ...
        // Why: launch tokens can outlive an TeamRun-started TUI in the shell; once the session is done they must no longer authorize config reuse.
      ...
      if (completionRefreshWorktreeId) {
        ...
        // Why: agents can create a PR via `gh pr create`, bypassing TeamRun's flow and leaving a stale "no PR" cache entry in place.
