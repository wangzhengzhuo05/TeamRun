...
const ORCA_FEATURE_REQUEST_URL = 'https://github.com/wangzhengzhuo05/TeamRun/issues/new'
...
export default function ProjectViewWrapper({ selectedRepoIds }: Props): React.JSX.Element {
  ...
  if (resolvedDialogRepoItem !== dialogRepoItem) {
    // Why: clear the repo-backed dialog when its repo leaves TeamRun, before the modal tree gets stale repo ids.
            ...
            <DialogDescription>
              {resolvedMissingRepoDialogs.repoNotInOrca
                ? translate(
                    ...
                    "{{value0}}/{{value1}} isn't added to TeamRun. Add it to start work, or open in GitHub.",
                    {
  ...
  return (
    <div className="project-view-tab-strip flex min-h-[41px] min-w-0 flex-none items-end gap-1 overflow-x-auto overflow-y-hidden border-b border-border/50 bg-muted/20 px-3 pt-3">
      {views.map((v) => {
            ...
            title={
              supported
                ...
                : translate(
                    ...
                    "{{value0}} — TeamRun doesn't support {{value1}} project views yet. File a feature request at {{value2}}.",
        ...
        const unsupportedMessage = `TeamRun doesn't support ${layoutLabel} project views yet.`
        return (
              ...
              <div className="space-y-2">
                <p className="text-xs leading-5 text-muted-foreground">
                  ...
                  {translate(
                    ...
                    'Switch to a Table view to work with this project in TeamRun.'
                  )}
                </p>
                <Button
                  type="button"
...
function ErrorState({
  ...
  const copy =
    error.type === 'too_large'
      ? `This view has ${totalCount ?? 'many'} items — too large to render in TeamRun. Narrow the view's filter on GitHub.`
      : error.type === 'unsupported_layout'
        ? 'TeamRun only renders table views yet. This is a Board or Roadmap view.'
