...
function unsetEnvVarCommand(varName: string): { label: string; command: string } {
  if (IS_WINDOWS) {
    ...
    // shell/TeamRun relaunch for the change to take effect.
    return {
      label: translate(
...
export function buildRemediation(
  ...
  if (!diag.ghAvailable) {
    return {
      ...
      detail:
        'TeamRun uses `gh` to talk to GitHub Projects. Install it from cli.github.com, then sign in.',
      commands: [
        {
          label: translate(
            'auto.components.github.project.GhAuthErrorHelp.9c2da6353b',
  ...
  if (active?.envToken) {
    ...
    return {
      ...
      detail: IS_WINDOWS
        ? `Find where \`${varName}\` is set (System or User environment variables, or your PowerShell profile), remove it, then restart TeamRun so the new environment is picked up.${fallback}`
        : `Find where \`${varName}\` is exported (commonly \`~/.zshrc\`, \`~/.zshenv\`, \`~/.bashrc\`, \`~/.profile\`, or your shell's secrets manager), remove it, then restart TeamRun so the new environment is picked up.${fallback}`,
  ...
  // separate terminal, TeamRun's gh subprocess sees the env var and uses it.
  if (diag.envTokenInProcess && (!active || diag.missingScopes.length > 0)) {
    const varName = diag.envTokenInProcess
    return {
      summary: `TeamRun inherited \`${varName}\` from your shell, and \`gh\` is using that token. \`gh auth refresh\` doesn't apply to env-supplied tokens.`,
      detail: `Unset \`${varName}\` in the shell that launches TeamRun${
        IS_WINDOWS ? ' (or in your user environment variables)' : ' (or in your shell rc file)'
      }, then restart TeamRun.`,
      commands: [findEnvVarCommand(varName), unsetEnvVarCommand(varName)],
      docsUrl: 'https://cli.github.com/manual/gh_help_environment'
    }
  }
