...
export function HostRemoveDialog({
  ...
  // error context owned by the TeamRun servers settings pane, so we deep-link
  ...
  const description =
    target.kind === 'runtime'
      ? translate(
          ...
          'This opens the TeamRun servers settings where you can remove this server.'
        )
      : hasWorkspaces
        ? translate(
            'auto.components.sidebar.HostRemoveDialog.hostHasWorkspacesDefault',
            'Removes {{value0}} and its credentials from this computer. Its {{value1}} stay in TeamRun — remote files are not touched.',
  ...
  // only forget TeamRun's records (offline/ghost host).
  const deleteOptionLabel = isConnected
    ...
    : translate(
        ...
        'Also remove these {{value0}} from TeamRun',
