 ...
 * workspace from TeamRun because the SSH target is gone entirely.
 */
export type SshWorkspaceForgetResolution =
  ...
  // impossible, so forget-from-TeamRun is the only path.
...
export function resolveSshWorkspaceForget(args: {
  ...
  // the only escape is to forget it from TeamRun.
  if (!isConfigured) {
