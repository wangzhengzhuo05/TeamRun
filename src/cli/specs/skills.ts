...
export const SKILL_COMMAND_SPECS: CommandSpec[] = [
  {
    path: ['skills', 'list'],
    summary: 'List version-matched skill guides bundled with this TeamRun CLI',
    usage: 'orca skills list [--json]',
    allowedFlags: [...GLOBAL_FLAGS],
    notes: [
      'Reads bundled guide metadata locally without contacting the TeamRun runtime.',
  ...
  },
  {
    ...
    notes: [
      'Reads bundled guide content locally without contacting the TeamRun runtime.',
  ...
  {
    path: ['skills', 'install'],
    summary: 'Install bundled TeamRun skills via the community skills CLI',
    usage:
      'orca skills install [--skill <name>]... [--all] [--agent <name>[,<name>]] ' +
      '[--local] [--dry-run] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'skill', 'all', 'agent', 'local', 'dry-run'],
    notes: [
      'Reads the bundled skill registry locally without contacting the TeamRun runtime.',
      'Resolves to the same `npx skills add <repo> --skill <name> ...` command used by ' +
        'TeamRun Settings, plus the non-interactive flags an unattended host needs ' +
      ...
      'Targets the coding agents TeamRun detects on this host, plus the shared ' +
      ...
      'Use --agent <name>[,<name>...] to choose targets yourself, or --agent universal ' +
        'for the shared directory alone. Required when TeamRun detects no agent.',
  ...
  {
    path: ['skills', 'update'],
    summary: 'Update already-installed TeamRun skills via the community skills CLI',
    usage: 'orca skills update [--skill <name>]... [--all] [--local] [--dry-run] [--json]',
    allowedFlags: [...GLOBAL_FLAGS, 'skill', 'all', 'local', 'dry-run'],
    notes: [
      'Reads the bundled skill registry locally without contacting the TeamRun runtime.',
      'Resolves to the same `npx skills update <names...>` command used by TeamRun Settings, ' +
