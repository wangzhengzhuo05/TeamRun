...
Startup:
  open                      Launch TeamRun and wait for the runtime to be reachable
  serve                     Start a headless TeamRun runtime server
  status                    Show app/runtime/graph readiness

Diagnostics:
  diagnostics memory        Collect a memory snapshot for TeamRun and managed terminals

Agent Discovery:
  agent-context             Print the machine-readable command schema for agents

Accounts:
  account add               Add a managed Claude or Codex account on this TeamRun host
  account list              List managed Claude and Codex accounts on this TeamRun host

Skills:
  skills list               List version-matched skill guides bundled with this TeamRun CLI
  skills get                Print a version-matched skill guide as Markdown
  skills install            Install bundled TeamRun skills globally via the community skills CLI
  skills update             Update already-installed TeamRun skills via the community skills CLI

Environments:
  environment add           Save a remote TeamRun runtime from a pairing code
  environment list          List saved remote TeamRun runtimes
  environment show          Show one saved remote TeamRun runtime
  environment rm            Remove a saved remote TeamRun runtime

Environment Recipes:
  vm recipe doctor          Validate a per-workspace environment recipe

Automations:
  automations list          List scheduled TeamRun automations
  automations show          Show one TeamRun automation
  automations create        Create a scheduled TeamRun automation
  automations edit          Edit an TeamRun automation
  automations remove        Remove an TeamRun automation and its run history
  automations run           Run an TeamRun automation now
  automations runs          List automation run history

Projects:
  project list              List durable projects known to TeamRun
...
Repos:
  repo list                 List repos registered in TeamRun
  repo add                  Add a project to TeamRun by filesystem path
...
Worktrees:
  worktree list             List TeamRun-managed worktrees
  worktree show             Show one worktree
  worktree current          Show the TeamRun-managed worktree for the current directory
  worktree create           Create a new TeamRun-managed worktree
  worktree set              Update TeamRun metadata for a worktree
  worktree rm               Remove a worktree from TeamRun and git
  worktree ps               Show a compact orchestration summary across worktrees

Files:
  file open                 Open a workspace file in the TeamRun editor
  file diff                 Open a workspace file diff in the TeamRun editor
  file open-changed         Open all git-changed files for a workspace

Terminals:
  terminal list             List live TeamRun-managed terminals
...
Orchestration:
  ...
  orchestration worker-start Start a supervised worker locally or on a connected TeamRun server
...
Mobile Emulator (iOS Simulator):
  emulator list             List available/running emulators (TeamRun-managed + raw serve-sim)
...
Output Options:
  ...
  --pairing-code <code>      Connect to a remote TeamRun runtime using an orca://pair?... code
  --environment <selector>   Connect using a saved environment id or name
  --help                    Show this help message

Behavior:
  Most commands require a running TeamRun runtime. If TeamRun is not open yet, run \`orca open\` first.
...
export function formatFlagHelp(flag: string): string {
  const helpByFlag: Record<string, string> = {
    ...
    comment: '--comment <text>       Comment stored in TeamRun metadata',
    ...
    activate: '--activate             Reveal the new worktree in the TeamRun app',
    ...
    'display-name': '--display-name <name>  Override the TeamRun display name',
    ...
    focus: '--focus                Reveal the created terminal session in TeamRun',
  ...
  if (flag === 'current') {
    return '--current              Use the current TeamRun worktree linked Linear issue'
  }
  if (flag === 'comments') {
