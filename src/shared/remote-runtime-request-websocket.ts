...
export function openRemoteRuntimeWebSocket(
  ...
  const onError = (): void => {
    callbacks.onError(
      ws,
      remoteRuntimeUnavailableError('Could not connect to the remote TeamRun runtime.')
    )
  }
  ...
  const cleanup = (): void => {
    ...
    // that from becoming an unhandled EventEmitter error after detaching TeamRun.
