...
/**
 * The community `skills` CLI's own `--agent` key for each agent TeamRun detects.
 *
 ...
 * rather than guessed. TeamRun ids and skills keys agree less often than they look
 * (`claude` is `claude-code`, `rovo` is `rovodev`, `aug` is `augment`), and some
 * near-matches are different products — TeamRun's `aider` CLI is not the CLI's
 * `aider-desk`, and TeamRun's `openclaude` is its `openclaw` in name only, so it
 * follows TeamRun's own rule that OpenClaude reads Claude-owned roots.
 */
export const SKILLS_CLI_AGENT_KEY_BY_TUI_AGENT = {
  claude: 'claude-code',
  'claude-agent-teams': 'claude-code',
  // Why: TeamRun states OpenClaude reads Claude-owned roots (native-chat-agent-profiles).
  ...
  devin: 'devin',
  ante: null,
  // Why: TeamRun detects trae by `traecli`, an alias only TRAE CN ships.
  trae: 'trae-cn'
} satisfies Record<TuiAgent, string | null>

/**
 * The shared `.agents/skills` target every universal agent reads. Always included
 * so agents TeamRun cannot map still receive the skill.
 */
...
/** Map detected TeamRun agents onto `skills --agent` keys, plus the universal target. */
