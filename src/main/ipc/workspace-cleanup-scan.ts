...
async function scanRepoWorkspaces(
  ...
  const candidatesWithSkipped = await mapWorkspaceCleanupWithConcurrency(
    ...
    async (worktree) => {
      // Why: externally-created worktrees can miss TeamRun activity stamps; local
...
async function listCleanupGitWorktrees(
  ...
  if (repo.connectionId) {
    ...
    if (!provider) {
      // Why: cleanup should reflect only workspaces TeamRun can currently inspect.
...
function handleRepoWorktreeListError(args: {
  ...
  if (repo.connectionId && !targetWorktreeId) {
    // Why: broad cleanup only shows remote workspaces TeamRun can inspect now.
