...
// macOS convention is `~/Library/Application Support/TeamRun/logs/`; Windows and
...
function getUserDataDir(): string {
  try {
    ...
    const home = homedir()
    if (platform() === 'darwin') {
      return join(home, 'Library', 'Application Support', 'TeamRun')
    }
    if (platform() === 'win32') {
      return join(process.env.APPDATA ?? home, 'TeamRun')
    }
    return join(home, '.config', 'TeamRun')
  }
}
