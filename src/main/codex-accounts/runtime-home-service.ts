...
export class CodexRuntimeHomeService {
  ...
  // Last auth.json TeamRun wrote to the runtime home; a later diff signals an out-of-band change (Codex token refresh, or external login to adopt).
  ...
  private initializeLastSyncedState(): void {
    ...
    // Why: WSL-managed homes never touch host ~/.codex; treating one as "last synced" makes cold start mangle host auth TeamRun never touched.
  ...
  prepareForCodexLaunch(
    ...
    if (selfContainedAccount) {
      ...
      // system default without injecting a path TeamRun cannot prove it owns.
    }
  ...
  // all. Hardlink every other TeamRun-visible home's rollouts in — after launch,
  ...
  getHostCodexHomePathsForSessionDiscovery(): string[] {
    ...
    if (this.isHostSystemDefaultRealHome() || this.getSelfContainedManagedHostAccount()) {
      // Why: nested TeamRun processes can retain an ambient managed CODEX_HOME.
  ...
  prepareForRateLimitFetch(target?: CodexAccountSelectionTarget): string | null {
    ...
    if (this.isHostSystemDefaultRealHome()) {
      ...
      // CODEX_HOME before ~/.codex. Nested TeamRun launches can inherit the
  ...
  syncForCurrentSelection(
    ...
    if (this.lastSyncedAccountId !== null) {
      ...
      if (logoutMarkerStatus.kind === 'applies') {
        ...
        // Why: unmanaged sessions use an TeamRun-owned CODEX_HOME; seed it once from system-default auth so terminals stay logged in without mutating ~/.codex.
        this.restoreSystemDefaultSnapshot({ detectExternalLogin: false })
      } else {
  ...
  private getPreparedWslRateLimitHomePath(target: CodexAccountSelectionTarget): string | null {
    ...
    if (distro) {
      ...
      if (selectedAccountId === null) {
        // Why: the system-default account changes outside TeamRun, so read its real home directly to avoid a stale cached runtime copy.
  ...
  private syncRuntimeAuthWithSystemDefault(): void {
    ...
    try {
      ...
      // Why: mirror external logins/logouts into TeamRun's runtime home so unmanaged Codex sessions keep matching the current system-default state.
      this.captureSystemDefaultSnapshot({ force: true })
      this.writeRuntimeAuth(systemDefaultAuth, { owner: 'system-default' })
    } catch (error) {
      ...
      const knownSharedAuth =
        provenance?.owner === 'system-default'
          ...
          : provenanceStatus.kind === 'missing'
            ...
            : null
      // Why: only bytes TeamRun can prove it wrote belong to the compatibility
  ...
  private restoreSystemDefaultSnapshot(options: { detectExternalLogin: boolean }): void {
    ...
    if (options.detectExternalLogin && !existsSync(runtimeAuthPath)) {
      // Why: with TeamRun owning CODEX_HOME, a deleted runtime auth.json is a local logout, not a cue to restore the user's real ~/.codex snapshot.
      this.persistRuntimeLogoutMarker()
      this.lastWrittenAuthJson = null
      this.persistSharedRuntimeAuthProvenance({ owner: 'system-default', authJson: null })
      return
  ...
  private clearRuntimeAuthAfterSystemDefaultLogout(runtimeAuthPath: string): void {
    // Why: a vanished ~/.codex auth means external logout for unmanaged sessions, even if runtime auth already refreshed in TeamRun's CODEX_HOME.
    rmSync(runtimeAuthPath, { force: true })
    this.captureSystemDefaultSnapshot({ force: true })
    this.persistRuntimeLogoutMarker()
    this.lastWrittenAuthJson = null
