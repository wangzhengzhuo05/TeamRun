...
export type SystemTrayOptions = {
  ...
  /** Quit TeamRun for real (caller must set the quitting latch before quitting). */
  onQuit: () => void
}
...
function baseTooltip(): string {
  ...
  return devIndicator.label ? `TeamRun DEV (${devIndicator.label})` : 'TeamRun DEV'
}
      ...
      try {
        ...
        tray.setToolTip(
          devIndicator
            ...
            : translateMain('tray.activityWaiting', 'TeamRun - activity waiting')
        )
        return
      } catch (error) {
...
export function createSystemTray(opts: SystemTrayOptions): Tray | null {
  ...
  const menu = Menu.buildFromTemplate([
    ...(devIndicator
      ...
      : []),
    {
      label: translateMain('tray.openOrca', 'Open TeamRun'),
