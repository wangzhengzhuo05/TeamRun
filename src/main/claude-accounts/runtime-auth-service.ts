...
export class ClaudeRuntimeAuthService {
  ...
  // Why: creds TeamRun last wrote to the shared file; a mismatch on managed→default transition means an external login overwrote it, so adopt it as the new default.
  ...
  private async doSyncForCurrentSelection(target?: ClaudeAccountSelectionTarget): Promise<void> {
    ...
    if (activeAccount.managedAuthRuntime === 'wsl') {
      if (!this.getOwnedManagedAuthPath(activeAccount)) {
        console.warn(
          '[claude-runtime-auth] Active WSL managed account is not owned by TeamRun, restoring system default'
        )
        const nextSelection = setSelectedClaudeAccountIdForTarget(
          normalizeClaudeRuntimeSelection(settings),
          null,
    ...
    if (!this.getOwnedManagedAuthPath(activeAccount)) {
      console.warn(
        '[claude-runtime-auth] Active managed account is not owned by TeamRun, restoring system default'
      )
      if (this.lastSyncedAccountId !== null) {
        if (
          previousAccount &&
  ...
  private getOwnedRuntimeOauthBaseline(
    ...
    // Why: managed metadata hints identity but isn't proof TeamRun wrote .claude.json; use only after a credential surface proves ownership.
