...
export function resolveMacOSComputerUseAppPath(): string | null {
  ...
  const packaged = [join(process.resourcesPath ?? '', 'TeamRun Computer Use.app')]
  const dev = [
    join(process.cwd(), 'native/computer-use-macos/.build/release/TeamRun Computer Use.app'),
    resolve(__dirname, '../../native/computer-use-macos/.build/release/TeamRun Computer Use.app')
  ]
