...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  if (target === 'local' && process.platform === 'win32') {
    return [
      '@echo off',
      'setlocal',
      // Why: endpoint file holds the live port/token; a PTY that outlives an TeamRun restart carries stale env, so `call` it to refresh (else PTY env).
  ...
  return [
    '#!/bin/sh',
    ...buildPosixHookPayloadCapture(),
    // Why: endpoint file holds the live port/token; PTYs that outlive an TeamRun restart carry stale env, so source it to reach the new server (else PTY env).
...
export class DevinHookService {
  ...
  async installRemote(sftp: SFTPWrapper, remoteHome: string): Promise<AgentHookInstallStatus> {
    ...
    try {
      ...
      // Why: SSH remotes use POSIX `.sh` hooks even when TeamRun runs on Windows; never derive remote script syntax from local OS.
