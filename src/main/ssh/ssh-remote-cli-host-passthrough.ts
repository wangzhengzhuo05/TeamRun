...
// switch (the cause of "Unsupported SSH TeamRun CLI command", #7716), the host
...
// Why: only TeamRun terminal-context vars may cross from the remote shell into
...
export async function runHostOrcaCliPassthrough(
  ...
  try {
    ...
    // reports "TeamRun is not running" against a healthy app.
  ...
  if (!entryExists(cliEntryPath)) {
    throw new HostCliUnavailableError(`TeamRun CLI entry not found at ${cliEntryPath}`)
  }
  ...
  return await new Promise<RemoteOrcaCliResult>((resolve, reject) => {
    ...
    const killTimer = setTimeout(() => {
      ...
      resolve({
        ...
        stderr: `${stderr.toString()}TeamRun CLI bridge timed out after ${killTimeoutMs}ms on the host.\n`,
        exitCode: 1
      })
    }, killTimeoutMs)
    killTimer.unref?.()
    ...
    child.on('error', (err) => {
      ...
      reject(
        new HostCliUnavailableError(`Failed to launch the TeamRun CLI on the host: ${err.message}`)
      )
    })
