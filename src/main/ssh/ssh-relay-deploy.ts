...
async function deployAndLaunchRelayAttempt(
  ...
  if (!hostPlatform) {
    throw new Error(
      'Unsupported remote platform. TeamRun relay supports: linux-x64, linux-arm64, darwin-x64, darwin-arm64, win32-x64, win32-arm64.'
    )
  }
  ...
  if (!localRelayDir) {
    throw new Error(
      `Relay package for ${platform} not found locally. ` +
        `This may be a packaging issue — try reinstalling TeamRun.`
    )
  }
...
/**
 * A marker is only meaningful where a split namespace can occur and where TeamRun
...
async function launchRelay(
  ...
  const escapedNode = shellEscape(nodePath)
  // Why: remoteRelayDir is shared across TeamRun targets for one account; hashing the target ID into the socket name stops cross-target attach.
