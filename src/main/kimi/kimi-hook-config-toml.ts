...
// so TeamRun manages only its own marker-delimited block: install rewrites the
...
// Why: mirror the Claude-compatible events TeamRun normalizes for status. Kimi uses
...
const BLOCK_START = '# >>> orca-managed-kimi-hooks (managed by TeamRun; do not edit) >>>'
...
// TeamRun generates; escape both defensively anyway.
...
// TeamRun-managed script (by filename, so a moved userData path is still swept).
