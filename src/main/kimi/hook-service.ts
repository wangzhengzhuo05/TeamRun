...
function getManagedScript(target: 'local' | 'posix' = 'local'): string {
  ...
  const endpointRefreshAndGuard = [
    // Why: refresh PORT/TOKEN/ENV/VERSION from the current TeamRun install so a PTY
    // that survived an TeamRun restart still reaches the live listener. See
...
export class KimiHookService {
  ...
  // Why: install TeamRun's managed Kimi hooks on a remote box over SFTP, mirroring
