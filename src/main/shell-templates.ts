...
export function getZshEnvTemplate(zshDir: string, headerPrefix = ''): string {
  const header = headerPrefix
    ? `TeamRun ${headerPrefix} zsh shell-ready wrapper`
    : 'TeamRun zsh shell-ready wrapper'
  return `# ${header}
...
# TeamRun PTYs never source another TeamRun wrapper recursively.
...
if [[ ${checks.join(' && ')} ]]; then
  ...
  # TeamRun restores its wrapper dir afterward so zsh still loads wrapper files.
...
# Why: after TeamRun's last wrapper file has loaded, the interactive shell should
