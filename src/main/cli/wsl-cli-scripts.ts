const MANAGED_MARKER = '# TeamRun managed WSL CLI launcher'
const BRIDGE_MANAGED_MARKER = '# TeamRun managed WSL CLI PowerShell bridge'
...
if command -v powershell.exe >/dev/null 2>&1; then
  ...
  echo "TeamRun WSL CLI requires Windows interop and could not find powershell.exe." >&2
  exit 1
fi
...
try {
  ...
  if ($args.Count -lt 1) {
    throw 'Invalid TeamRun WSL CLI bridge invocation.'
  }
  ...
  if ($null -eq $Process) {
    throw 'Unable to start the TeamRun Windows CLI launcher.'
  }
  $Process.WaitForExit()
...
export function buildRegistrationLockPrelude(commandPath: string): string {
  ...
  // Why: the per-distro queue only serializes one TeamRun process; flock covers
...
export function buildManagedLegacyRemoveCommand(quotedLegacyCommandPath: string): string {
  // Why: remove only the TeamRun-managed pre-rename wrapper; user-owned `orca`
