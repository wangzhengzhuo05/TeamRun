...
export class CliInstaller {
  ...
  private get commandName(): string {
    ...
    // Why: packaged Linux uses `orca-ide` to avoid shadowing GNOME TeamRun's /usr/bin/orca.
    ...
    if (!launcherPath) {
      const detail =
        this.isLinuxAppImage() && this.appImagePath
          ...
          : this.isPackaged
            ? 'The bundled CLI launcher is missing from this TeamRun build.'
  ...
  async remove(): Promise<CliInstallStatus> {
    ...
    if (status.state === 'stale') {
      throw new Error(`Refusing to remove a command not owned by TeamRun at ${status.commandPath}.`)
    }
    ...
    for (const commandPath of this.getPathCommandCandidates(defaultCommandPath)) {
      ...
      if (status.state !== 'not_installed') {
        if (reachedDefaultCommandPath && !isDefaultCommandPath && status.state === 'conflict') {
          // Why: a non-TeamRun command after an empty default slot can be shadowed by installing there; no user file replaced.
          continue
        }
  ...
  private resolveCommandPath(): string | null {
    ...
    if (!this.isPackaged) {
      ...
      if (this.platform === 'win32') {
        return join(this.localAppDataPath, 'Programs', 'TeamRun Dev', 'bin', `${DEV_COMMAND_NAME}.cmd`)
      }
    }
    ...
    if (this.platform === 'linux') {
      ...
      // Why `orca-ide`: GNOME TeamRun ships /usr/bin/orca, so avoid shadowing that screen reader.
  ...
  private async removeLegacyLinuxCommandIfManaged(launcherPath: string | null): Promise<void> {
    ...
    try {
      ...
      // Why: after the Linux command rename, the old `orca` symlink would keep shadowing GNOME TeamRun.
    ...
    try {
      ...
      if (!stats.isFile()) {
        return this.buildStatus({
          ...
          state: 'conflict',
          currentTarget: null,
          detail: `${commandPath} exists but is not an TeamRun launcher script.`
        })
      }
      ...
      if (isMissingError(error)) {
        return this.buildStatus({
          ...
          state: 'not_installed',
          currentTarget: null,
          detail: `Register ${commandPath} to use TeamRun from the terminal.`
        })
      }
      throw error
    }
    ...
    try {
      ...
      if (!stats.isSymbolicLink()) {
        if (stats.isFile()) {
          ...
          if (managedTarget) {
            return this.buildStatus({
              ...
              detail: `${commandPath} contains an older TeamRun launcher.`
            })
          }
        }
        ...
        return this.buildStatus({
          ...
          state: 'conflict',
          currentTarget: null,
          detail: `${commandPath} exists but is not an TeamRun symlink.`
        })
      }
      ...
      return this.buildStatus({
        ...
        detail: isInstalled
          ...
          : isManagedStaleTarget
            ? `${commandPath} points to an older TeamRun launcher.`
            : `${commandPath} points to a non-TeamRun launcher.`
      })
    } catch (error) {
      if (isMissingError(error)) {
        return this.buildStatus({
  ...
  private isManagedSymlinkTarget(resolvedTarget: string, launcherPath: string): boolean {
    ...
    if (this.platform === 'darwin') {
      // Why: reclaim symlinks to an older TeamRun.app launcher, but never replace arbitrary user-owned symlinks.
  ...
  private isSiblingDevLauncherTarget(
    ...
    // Why: dev builds generate launchers under the sibling `*-dev` profile; packaged TeamRun must reclaim that command.
    return (
    ...
    try {
      ...
      if (isMissingError(error)) {
        return this.buildStatus({
          ...
          state: 'not_installed',
          currentTarget: null,
          detail: `Register ${commandPath} to use TeamRun from Command Prompt or PowerShell.`
        })
      }
      throw error
    }
  ...
  private withPathInfo(
    ...
    if (
      ...
      return {
        ...
        state: 'not_installed',
        currentTarget: null,
        detail: `Register ${status.commandPath} to use TeamRun from Command Prompt or PowerShell.`
      }
    }
    ...
    if (pathConfigured === null) {
      return {
        ...
        detail:
          pathProbe.detail ??
          'The TeamRun launcher exists, but TeamRun could not check your Windows user PATH.'
      }
    }
  ...
  private async writeWindowsUserPathEntry(
    ...
    try {
      ...
      const guidance =
        action === 'add'
          ? `Add this folder to your PATH manually: ${pathDirectory}. Or run TeamRun as an administrator and try again.`
          : `Remove this folder from your PATH manually: ${pathDirectory}. Or run TeamRun as an administrator and try again.`
      throw new Error(
...
function extractManagedUnixLauncherTarget(content: string): string | null {
  ...
  // Why: only TeamRun's compiled CLI entrypoints count as managed; arbitrary Electron-launching scripts stay conflicts.
