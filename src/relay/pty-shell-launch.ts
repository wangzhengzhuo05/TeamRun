...
function ensureOverlayRestoreWrappers(root: string): void {
  ...
  const zshEnv = `# TeamRun relay zsh overlay wrapper
${SHELL_STARTUP_IDENTITY_MARKER_BLOCK}
...
`
  const zshProfile = `# TeamRun relay zsh overlay wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zprofile',
  homeExpression: '"${ORCA_USER_ZDOTDIR:-${ORCA_ORIG_ZDOTDIR:-$HOME}}"'
})}
`
  const zshRc = `# TeamRun relay zsh overlay wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zshrc',
  homeExpression: '"${ORCA_USER_ZDOTDIR:-${ORCA_ORIG_ZDOTDIR:-$HOME}}"',
...
`
  const zshLogin = `# TeamRun relay zsh overlay wrapper
${getZshStartupFileSourceBlock({
  fileName: '.zlogin',
  homeExpression: '"${ORCA_USER_ZDOTDIR:-${ORCA_ORIG_ZDOTDIR:-$HOME}}"',
  interactiveOnly: true
...
`
  const bashRc = `# TeamRun relay bash overlay wrapper
${SHELL_STARTUP_IDENTITY_MARKER_BLOCK}
...
__orca_normalize_prompt_command() {
  ...
  # a ";"/whitespace separator; trim it so TeamRun's prepend/append never form ";;".
