...
TeamRun executes the user's Git binary on three kinds of execution host: native,
...
is the oldest line that covers TeamRun's baseline use of porcelain v2, `branch
--show-current`, `restore`, and sparse checkout. Optional features that need a
newer Git must degrade safely and cache the missing capability. TeamRun does not
...
| `for-each-ref-exclude`  | Exclude remote HEAD before the output limit       | Request extra refs, then filter remote HEAD in TeamRun                                     |
...
flag work on an older binary or choose TeamRun's semantic fallback automatically.
It provides version reporting and subprocess queueing, but TeamRun already needs
