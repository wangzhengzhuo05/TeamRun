...
export function ProtocolBlockScreen({ verdict }: Props) {
  ...
  const title = isMobileTooOld ? 'Update TeamRun Mobile' : 'Update TeamRun on your computer'
  const body = isMobileTooOld
    ? `This desktop needs a newer TeamRun Mobile app. Update TeamRun Mobile from ${mobileUpdateTarget.storeName}, then try this host again.`
    : 'This paired desktop app is too old for your current TeamRun Mobile app. Update TeamRun on your computer, then try this host again.'
  const recoveryNote =
