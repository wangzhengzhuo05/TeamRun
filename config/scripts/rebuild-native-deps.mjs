  ...
  if (isWindowsNativeLockError(err)) {
    console.error(
      '[rebuild] A Windows process appears to be using a native .node file. ' +
        'Close running TeamRun/Electron/dev processes for this worktree, then rerun `pnpm install` ' +
...
function getPatchedNodePtyRebuildReason() {
  ...
  // Why: TeamRun patches node-pty's native Unix spawn path; upstream prebuilds can
    ...
    if (requirePatchedNodePtySourceBuild && !isNodePtyReleaseBuildDir(native.dir)) {
      throw new Error(
        'node-pty resolved to ' +
          native.dir +
          '; expected build/Release so TeamRun\\'s node-pty patch is active'
      )
    }
    return
  }
